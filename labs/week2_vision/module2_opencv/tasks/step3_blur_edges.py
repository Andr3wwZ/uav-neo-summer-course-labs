...
def update(drone):
    ...
    blurred = cv2.GaussianBlur(gray_im, (KERNEL_SIZE, KERNEL_SIZE), 0)

    sobel_x = cv2.Sobel(blurred, cv2.CV_64F, dx=1, dy=0, ksize=KERNEL_SIZE)

    sobel_y = cv2.Sobel(blurred, cv2.CV_64F, dx=0, dy=1, ksize=KERNEL_SIZE)
